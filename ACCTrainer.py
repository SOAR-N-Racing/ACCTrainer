...
class SharedMemoryReadError(SharedMemoryError):
    pass

class SharedMemoryWriteError(SharedMemoryError):
    pass

class SharedMemoryTimeout(SharedMemoryError):
    pass
...
def read_physic_map(physic_map: accSM) -> PhysicsMap:
    try:
        ...
        temp["absVibrations"],
            ...
            raise SharedMemoryReadError("Failed to read from physical_map")
...
def read_graphics_map(graphic_map: accSM) -> GraphicsMap:
    try:
        ...
        raise SharedMemoryReadError("Failed to read from graphics_map")
...
def read_static_map(static_map: accSM) -> StaticsMap:
    try:
        ...
        raise SharedMemoryReadError("Failed to read from static_map")   
...
class accSharedMemory():
    ...
    def read_shared_memory(self) -> Optional[ACC_map]:
        try:
            ...
            print("Eror accessing shared memory")
    ...
    def read_shared_memory_with_retry(self, retries=3):
        for _ in range(retries):
            try:
                return self.read_shared_memory()
            except SharedMemoryReadError:
                    time.sleep(1)
        raise SharedMemoryReadError("Failed after 3 retries")
...
def simple_test() -> None:
    ...
    for i in range(1000):
        ...
        if sm is not None:
            ...
            #logger.info(f"Graphics: Strategy tyre set: {sm.Graphics.penalty.name} | ")
